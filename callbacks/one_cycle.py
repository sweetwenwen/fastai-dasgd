...
class OneCycleScheduler(LearnerCallback):
    ...
    def __init__(self, learn:Learner, lr_max:float, moms:Floats=(0.9,0.9), div_factor:float=25., pct_start:float=0.3, final_div:float=None, tot_epochs:int=None, start_epoch:int=None, tao:int=None):
        super().__init__(learn)
        self.lr_max, self.div_factor, self.pct_start, self.final_div = lr_max, div_factor, pct_start, final_div
        if self.final_div is None: self.final_div = div_factor*100
    ...
    def on_train_begin(self, n_epochs:int, epoch:int, **kwargs:Any)->None:
        ...
        n = len(self.learn.data.train_dl) * self.tot_epochs * self.tao if self.tao is not None else len(self.learn.data.train_dl) * self.tot_epochs # iteration * epoch * tao
        ...
        # self.phases = ((a1,  annealing_step_function), (a2, annealing_step_function))
        # self.lr_scheds = self.steps((0, self.lr_max), (self.lr_max, self.lr_max/self.final_div))
        
        self.mom_scheds = self.steps(self.moms, (self.moms[1], self.moms[0]))
        self.opt = self.learn.opt
        self.opt.lr = self.lr_scheds[0].start
        self.opt.mom = self.mom_scheds[0].start
        self.idx_s = 0
        return res
